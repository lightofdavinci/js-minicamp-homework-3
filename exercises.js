/*
  ...
  for (var "A variable that iterates over the properties of an object" in "the specified object that will be iterated") {
    if (obj.hasOwnProperty(key)) {  // to make sure that the key you get is an actual property of an object, and doesn't come from the prototype
...
function hasEmail(user) {
  ...
  return Boolean(user.email); // or !!user.email
}
...
function hasProperty(object, property) {
  ...
  // otherwise return false
  return Boolean(object[property]); // or !!object[property];
}
...
function sumUserPostLikes(user) {
  ...
  // return the sum
  var sum = 0;
  user.posts.forEach(function(obj){ sum += obj.likes; });
  return sum;
}
